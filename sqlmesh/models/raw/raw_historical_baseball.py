...
def execute(context, start, end, **kwargs):
  ...
  with minio_client.get_object(MINIO_BUCKET_NAME, "baseball/historical/columns/historical_baseball_headers.csv") as response:
    columns = pl.read_csv(response, infer_schema=False, has_header=False)
    all_years.columns = columns.row(0)
